  ...
  useCopilotAction({
    ...
    render: ({ status, result }: { status: string; result?: { events: MarketMemorySummary["recentEvents"]; total: number } }) => {
      ...
      const schema = [
        ...
        { key: "swarmdaq:events:{id}",       type: "LIST",        desc: "Per-mission replay log. Reverse-chron." },
        { key: "swarmdaq:stream:market-events", type: "STREAM",   desc: "Append-only market events: bids, selections, evals, anomalies." },
        { key: "swarmdaq:stream:evaluations", type: "STREAM",     desc: "Evaluation-focused stream for score and reputation events." },
        { key: "swarmdaq:market:feed",       type: "LIST",        desc: "Rolling market ticker. Capped at 100 events." },
        { key: "swarmdaq:agent:{id}",        type: "HASH",        desc: "Live agent state: reputation, Bayesian α/β, Elo, factuality, cost." },
        { key: "swarmdaq:agent:{id}:history",type: "LIST",        desc: "Per-agent score history. Last 20 runs." },
        { key: "swarmdaq:agent:leaderboard:reputation", type: "SORTED SET", desc: "Reputation leaderboard." },
        { key: "swarmdaq:agent:leaderboard:elo", type: "SORTED SET", desc: "Elo leaderboard." },
        { key: "swarmdaq:agent:leaderboard:factuality", type: "SORTED SET", desc: "Factuality leaderboard." },
        { key: "swarmdaq:agent:leaderboard:market-value", type: "SORTED SET", desc: "Composite market value leaderboard." },
        { key: "swarmdaq:tdigest:price:global", type: "TDIGEST/LIST", desc: "Global clearing price percentile model." },
        { key: "swarmdaq:tdigest:price:task:{type}", type: "TDIGEST/LIST", desc: "Task clearing price percentile model." },
