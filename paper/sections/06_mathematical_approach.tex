...
For task \(t\), agent \(i\) forms a utility bid from expected quality, confidence, trust, cost, latency, and risk. Let \(s_{i,t}\in[0,1]\) be the fraction of required skills covered by the agent, \(p_i\in[0,1]\) be the agent's confidence, and \(h_i\in[0,1]\) be its normalized hallucination-risk estimate. The implementation estimates expected quality as
\begin{equation}
\hat{q}_{i,t} =
\mathrm{clip}_{[0,1]}
\left(
...
The final routing score combines the above components. To avoid long variable names in the equations, write \(z_i\) for the \marketmaker{} score, \(e_i\) for the UCB exploration score, \(\rho_i\) for normalized Elo, \(d_i\) for collaboration history, and \(f_i\) for factuality:
\begin{align}
z_i =
&\; w_s s_{i,t}
+ w_r r_i
...
-0.10\,u_i.
\end{align}
Each term is bounded or normalized to keep the score interpretable. Skill match \(s_{i,t}\) anchors the routing decision in task relevance. Reputation \(r_i\) and UCB score \(e_i\) balance historical trust with exploration. Bid utility \(b_{i,t}\) captures agent-specific confidence, quality, cost, latency, and risk. Elo \(\rho_i\) provides pairwise strength. Graph trust \(g_i\) rewards useful collaborators. Cost, latency, and uncertainty penalize expensive or unreliable choices.
